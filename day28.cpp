...
int main()
{
    ...
    std::regex base("([a-z.]+)@gmail\\.com");
    std::vector<string> filteredVector;
    ...
    for (int N_itr = 0; N_itr < N; N_itr++) {
        ...
        if(std::regex_match(emailID, base)) {
            filteredVector.push_back(firstName);
        }
    }
    ...
    for (int i = 0; i < filteredVector.size(); ++i) {
        std::cout << filteredVector[i] << std::endl;
    }
